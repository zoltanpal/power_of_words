...
from app.models.feeds import FeedDBFilters, Feeds
...
def get_sentiment_grouped(filters: FeedDBFilters, group_by: str = 'source_id', order_by: str = 'source_id ASC'):
    text_like = ""
    words_in = ""
    print(filters.conditions)
    if filters.free_text:
        text_like = f" AND lower(title) LIKE lower('%{filters.free_text}%')"
    if filters.words:
        print(filters.words)
        words_in = f" AND (words @> ARRAY{filters.words})"
...
def generate_sentiment_by_source_series(input_data):
    ...
    for key in sorted(set(negative_data.keys()) | set(neutral_data.keys()) | set(positive_data.keys())):
...
def get_most_common_words(filters, most_common: int = 20):
    ...
    for row_words in list(cursor_result):
        [words.append(word) for word in row_words[0] if word not in IGNORED_WORDS]
...
def index():
    ...
    sentiment_by_source_series = generate_sentiment_by_source_series(sentiment_by_source)

    sentiment_by_time = get_sentiment_grouped(filters=filters, group_by='feed_date', order_by='feed_date')
    sentiment_by_time_series = generate_sentiment_by_source_series(sentiment_by_time)

    sentiment_by_time_series_dates = {row[0].strftime("%Y-%m-%d") for row in sentiment_by_time}
